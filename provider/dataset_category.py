...
from utils.data_collate import precompute_data
...
class TrainingDataset(Dataset):
    def __init__(self,
        ...
        self.voxel_size = config.init_voxel_size
        self.search_radius = config.init_radius
        try: 
    ...
    def _read_instance_cat(self, index,cat):
        ...
        #import pdb;pdb.set_trace()
        pcd_points = pts.astype(np.float32)
        pcd_feats = np.ones((pcd_points.shape[0], 1), dtype=np.float32)
        pcd_points = torch.FloatTensor(pcd_points)
        pcd_feats = torch.FloatTensor(pcd_feats)
        lengths = torch.LongTensor([pcd_points.shape[0]])

        hist_n = int(np.ceil(4 / 3 * np.pi * (self.search_radius / self.voxel_size + 1) ** 3))    
        neighbor_hists = np.zeros((self.num_stages, hist_n), dtype=np.int32)
        #max_neighbor_limits = [hist_n] * self.num_stages     
        max_neighbor_limits = [85,30,30,27]
        points_list, lengths_list, neighbors_list, subsampling_list, upsampling_list = precompute_data(pcd_points, lengths, self.num_stages, self.voxel_size, self.search_radius, max_neighbor_limits)
        ...
        ret_dict['pcd_feats'] = pcd_feats
        ret_dict['pcd_points'] = points_list
        ret_dict['lengths'] = lengths_list
        ret_dict['neighbors'] = neighbors_list
        ret_dict['subsampling'] = subsampling_list
        ret_dict['upsampling'] = upsampling_list
        
        #ret_dict['pcd_points'] = pcd_points
        #ret_dict['pcd_feats'] = pcd_feats
        ...
        if instance_id == -1:
            ...
            for j in range(num_instance):
                ...
                if np.sum(mask) > 16:
                    ...
                    #import pdb;pdb.set_trace()
                    pcd_points = instance_pts.astype(np.float32)
                    pcd_feats = np.ones((pcd_points.shape[0], 1), dtype=np.float32)
                    pcd_points = torch.FloatTensor(pcd_points)
                    pcd_feats = torch.FloatTensor(pcd_feats)
                    lengths = torch.LongTensor([pcd_points.shape[0]])
                    ...
                    all_pcd_feats.append(pcd_feats.unsqueeze(0))
                    all_pcd_points.append(points_list)
                    all_lengths_list.append(lengths_list)
                    all_neighbors_list.append(neighbors_list)
                    all_subsampling_list.append(subsampling_list)
                    all_upsampling_list.append(upsampling_list)
            ...
            if len(all_pts) == 0:
                ...
                ret_dict['pcd_feats'] = torch.cat(all_pcd_feats, dim=0)
                ret_dict['pcd_points'] = all_pcd_points
                ret_dict['lengths'] = all_lengths_list
                ret_dict['neighbors'] = all_neighbors_list
                ret_dict['subsampling'] = all_subsampling_list
                ret_dict['upsampling'] = all_upsampling_list
